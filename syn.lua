...
-- Scaled size: takes up 38% width on PC, 75% on Mobile, with strict clamp limits
MainFrame.Size = UDim2.new(0.38, 0, 0.72, 0)
MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
MainFrame.AnchorPoint = Vector2.new(0.5, 0.5) -- Dynamic anchoring
...
-- Responsive size constraints (MainFrame won't overflow on big screens or shrink too much on phones)
local sizeConstraint = Instance.new("UISizeConstraint")
sizeConstraint.MinSize = Vector2.new(290, 380) -- Perfect fit for smallest phones
sizeConstraint.MaxSize = Vector2.new(440, 540) -- Perfect fit for PC monitors
...
-- Animate background glows elegantly
spawn(function()
...
ContentFrame.AutomaticCanvasSize = Enum.AutomaticCanvasSize.Y -- Dynamic calculation!
...
statBoxesLayout.Padding = UDim.new(0, 6) -- Dynamic responsive gap
...
local function CreateStatBox(parent, icon, label, value, color)
    ...
    box.Size = UDim2.new(0.33, -4, 1, 0) -- Auto-scalable width division
    box.ZIndex = 5
    box.Parent = parent
    CreateCorner(box, 10)
...
local function CreateSection(title, icon, layoutOrder)
    ...
    section.Size = UDim2.new(1, 0, 0, 0) -- Auto height based on internal elements
...
-- Auto Parry Section
local parrySection = CreateSection("AUTO PARRY CONTROLLER", "⚔", 2)
...
-- Visual Section
local visualSection = CreateSection("ESP & INTERFACE VISUALS", "👁", 3)
...
-- Settings Section
local settingsSection = CreateSection("SYSTEM SOUNDS", "⚙", 4)
...
MainFrame.Size = UDim2.new(0.38, 0, 0, 0) -- Starts thin responsively
